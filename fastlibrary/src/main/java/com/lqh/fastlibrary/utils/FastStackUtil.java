package com.lqh.fastlibrary.utils;
...
public class FastStackUtil {
    ...
    public Activity getCurrent() {
        if (mActivityStack != null && mActivityStack.size() != 0) {
            ...
            LauLogger.i(TAG, "get current activity:" + activity.getClass().getSimpleName());
            return activity;
        } else {
            return null;
        }
    ...
    public Activity getPrevious() {
        if (mActivityStack != null && mActivityStack.size() >= 2) {
            ...
            LauLogger.i(TAG, "get Previous Activity:" + activity.getClass().getSimpleName());
            return activity;
        } else {
            return null;
        }
    ...
    public FastStackUtil push(Activity activity) {
        ...
        LauLogger.i(TAG, "push stack activity:" + activity.getClass().getSimpleName());
        return sInstance;
    }
    ...
    public FastStackUtil pop(Activity activity, boolean isFinish) {
        if (activity != null) {
            LauLogger.i(TAG, "remove current activity:" + activity.getClass().getSimpleName() + ";isFinishing" + activity.isFinishing());
            if (mActivityStack != null && mActivityStack.contains(activity)) {
                mActivityStack.remove(activity);
                LauLogger.i(TAG, "remove current activity:" + activity.getClass().getSimpleName() + ";size:" + mActivityStack.size());
            }
            if (isFinish) {
                activity.finish();
            }
    ...
    public FastStackUtil exit(boolean kill) {
        try {
            ...
            System.exit(-1);
            LauLogger.e(TAG, "exit():" + e.getMessage());
        }
        return sInstance;
    }
}
