    ...
    public ErrorInfo(Throwable throwable) {
        ...
        if (throwable instanceof UnknownHostException) {
            if (!NetworkUtil.isConnected(Utils.getApp())) {
                errorMsg = getString(R.string.fast_exception_network_not_connected);
            } else {
            ...
            LogUtils.d("getLocalizedMessage: " + code);
            if ("416".equals(code)) {
                errorMsg = "请求范围不符合要求";
            } else if ("401".equals(code)) {
                errorMsg = "登录失效,请重新登录!";
//                ActivityUtils.startActivity(TipActivity.class);
    ...
    public boolean show() {
        ToastUtil.showFailed(TextUtils.isEmpty(errorMsg) ? throwable.getMessage() : errorMsg);
        return true;
    }
    ...
    public boolean show(String standbyMsg) {
        ToastUtil.showFailed(TextUtils.isEmpty(errorMsg) ? standbyMsg : errorMsg);
        return true;
    }
    ...
    public boolean show(int standbyMsg) {
        ToastUtil.showFailed(TextUtils.isEmpty(errorMsg) ? StringUtils.getString(standbyMsg) : errorMsg);
        return true;
    }
